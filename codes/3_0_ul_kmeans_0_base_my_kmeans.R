...
imageFile    = "data/images/tulliobuzzi_smaller.jpg"
...
myKmeans = function(df, K, iter.max = 5){
  iter = 0
  N = dim(df)[1] # observations
  M = dim(df)[2] # no. features
  centroidsIndexesForInit = sample(1:N, K, replace=F)
  centers = df[centroidsIndexesForInit,]
  cluster = rep(1, N)
  updated = TRUE
  
  while(updated && iter < iter.max){ # arrest criterions
    iter=iter+1
    print(iter)
    
    updated = FALSE # initialized to false
    for(i in 1:N){
      dists = rep(0, K)
      
      newCluster = 1
      for(k in 1:K){
        dists[k] = euclideanDist(df[i,], centers[k,])
        
        if(k!=newCluster && dists[k]<dists[newCluster]){
          newCluster = k
        }
      }
      # here newCluster is correctly been assigned
      
      if(!updated && newCluster != cluster[i]){
        updated = TRUE
      }
    ...
    # update centroids
    centers[]=0 # resets centroids
    # cumulate the sum inside of centroids matrix
    for(i in 1:N){
      k = cluster[i]
      centers[k,]=centers[k,]+df[i,]
    }
    
    # compute the mean starting from the cumulated sum
    for(k in 1:K){
      count = sum(cluster==k)
      centers[k,] = centers[k,]/count
    }
