...
######################################################
######################################################
######## IRIS DATASET KNN EVALUATE ACCURACY   ########
######################################################
######################################################
...
train_idx <- sample(1:nrow(iris), 0.9 * nrow(iris)) 

##extract training set
iris_train <- iris[train_idx, c(1,2,3,4)] 
##extract testing set
iris_test <- iris[-train_idx, c(1,2,3,4)] 
##extract 5th column of train dataset because it will be used as 'cl' argument in knn function.
iris_target_category <- iris[train_idx, 5]
##extract 5th column if test dataset to measure the accuracy
iris_test_category <- iris[-train_idx, 5]


## load the package class (to call knn method for classification)
library(class)
##run knn function
...
##########################################################
##########################################################
######## DIAMONDS DATASET KNN EVALUATE ACCURACY   ########
##########################################################
##########################################################
# 
...
# 
# ##the actual values of 2nd coulumn of testing dataset to compare it with values that will be predicted
# ##also convert ordered factor to normal factor
